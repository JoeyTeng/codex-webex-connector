...
`{cbth_bin}` 会绑定到 `--cbth-bin` 解析后的 executable，所以 W9 可以显式传入包含 C8/C9 的 cbth binary，而不依赖 `PATH` 上的 `cbth`。`--manifest-path` 可由 C9 接受为绝对路径或相对 `--release-dir` 的路径；W8 默认使用 release B 下的绝对 manifest 路径。repo 自带 `plugin/manifest.json` 保留 Webex packaging metadata；显式传入的 `--release-a` / `--release-b` 源 artifact 只要求 release 结构完整且 `plugin/manifest.json` 是目录内可读 JSON object，可以是缺少 `enabled=true` 的 legacy packaging metadata。live run 会先把显式 release 复制到 `test_root/releases/`，然后仅在默认 C9 command 或显式 `plugin upgrade` command 的 staged release B 中把同一路径补写为 C9-compatible `PluginManifest` object，包含 `name`、`executable_path`、`args`、`enabled=true`、`release_id`、string-array `capabilities` 和 task-scoped `environment`。后续 manifest 写入和 cbth 执行只使用该 task-scoped copy，不改写原始 release artifact；如果 `plugin/manifest.json` 的真实路径经 symlink 解析后离开 release 目录，harness 会在写入前 fail closed。不满足 C9 schema 时，W7 harness 会在执行 upgrade 前 fail closed；非 `plugin upgrade` 自定义 command 不会被强制改写成 C9 manifest 形态。
...
该 command 只展开 `{plugin}`、`{release_a}`、`{release_b}`、`{release_a_id}`、`{release_b_id}`、`{cbth_home}`、`{cbth_bin}` 和 `{prefix}`，并以关闭 stdin、有界 timeout、stdout/stderr 写入私有 `logs/webex-release-upgrade.log` 的方式执行。webex-connector 只模板化调用 C9 operator command、为 C9 `plugin upgrade` 生成 Webex release B 所需的 C9 manifest 输入，并验证 Webex product behavior；非 C9 custom hook 保留其 release artifact manifest 语义。不复制 cbth generic release manager，不直接编辑 cbth registry；registry 只能由 cbth service-side promote 写入。command 成功后，W7 会只读校验 task-scoped cbth registry 已切到 release B。
