...
- Sidecar 现在解析 worker ingress ACK，不再把 worker 拒收当作投递成功。Webex callback 入口会记录 `sidecar_received_at`，并在 `CBTH_PLUGIN_HOME/webex-sidecar-drain-state/` 下写入按 plugin instance/release/pid scoped 的 drain-state mirror；worker drain/shutdown 只读取当前 instance/release 的 live sidecar backlog 并计入 in-flight，忽略并清理已退出 sidecar 的 stale drain-state 文件，只放行严格早于首次 quiesce/shutdown cutoff 毫秒的 callback，并要求 sidecar 在 cutoff 后观测到 worker inactive 且停止监听后才信任 0 backlog，避免 drain 抢在 sidecar state write 前完成。cutoff 后的新 Webex work 会被拒绝且不会在 sidecar 内无限 retry。Sidecar 在无法持久化 drain-state 时会 fail closed，不继续处理缺少可见 drain-state 的 Webex callback；在 lifecycle rejection race 中会把已取到的 Webex ingress envelope 持久化到 `CBTH_PLUGIN_HOME/webex-sidecar-deferred-ingress/`，恢复 active 后按原始 `created` / `deferred_at` 顺序先 replay 这些 envelope，再恢复监听。
