...
- `plugin.shutdown` 成功时会写入按 plugin instance/release scoped 的 supervisor shutdown marker；supervisor 会先校验 marker instance/release，匹配时正常退出而不是重启旧 worker，避免 upgrade shutdown 被 supervisor 误判为可恢复 worker crash。
