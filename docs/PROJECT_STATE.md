...
- W7 opt-in live upgrade E2E harness 已落地：`scripts/w7_live_upgrade_e2e.py` 默认 dry-run，live 模式先执行 cbth C8 `service upgrade-smoke` safe harness，再隔离真实 Webex rooms、task-scoped cbth service/plugin home、session turn、delivery smoke、pagination-aware/idempotent cleanup/manifest；默认 PR CI 会运行安全的 W7 Python harness 单测和 dry-run；真实 Webex release A/B upgrade command 是 optional hook，并会验证 cbth registry 已切到 release B。
