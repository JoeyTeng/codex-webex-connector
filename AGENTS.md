...
* 你正在协助的对象是 **JoeyTeng**。
* 假设 JoeyTeng 是一名经验丰富的资深视频编码 / 视频处理 / 图像处理工程师，熟悉 C++、Rust、Go、Python 等主流语言及其生态。
* JoeyTeng 重视“Slow is Fast”，关注点在于：推理质量、抽象与架构、长期可维护性，而不是短期速度。
* 你的核心目标：
...
* 构建/依赖：默认 `git clone --recursive`，`meson` 固定 Python 3.12，区分 `Debug` 与 `Debug-RebuildDeps`。
* 提交/测试：每步包含代码/测试/文档，提交使用 `-S`，多行 message 用 `-m` 或 `-F`。
* 提交/测试：只要有代码修改（包含测试修改），提交前必须先运行测试并记录结果。
* 测试实例生命周期：凡是为 smoke/e2e/手工回归显式拉起测试用 `HDRStreamer.app` 实例，结束后都必须显式退出；若同时拉起 `HDRColorSourceApp` 等 source app，也必须一并收尾。
* Xcode DerivedData：需要自定义 `-derivedDataPath` 时，优先使用 `scripts/xcodebuild_with_derived_data.sh`。默认使用 task-scoped 临时目录并在脚本结束后自动清理；只有 live e2e 等必须复用固定签名产物路径的场景，才显式保留，并在总结里报告保留路径与原因。
* TCC / sharing-sensitive 路径：凡是依赖固定签名产物路径稳定 `Screen Recording` / sharing 等 TCC 权限的 HDRStreamer workflow，优先使用 `run_automation_e2e_signed.sh` 或同一固定 signed-path 封装，不要再手写分散的 `xcodebuild build ... -derivedDataPath /tmp/HDRStreamerDerivedData-*` + `open` + 手工清理链路。默认入口应负责 fixed-path 冲突预检、按本次拉起实例的 PID teardown，并避免按应用名误操作到用户已有实例。
* 日志治理：先分类再处理，高频日志限频或只打印一次。
* GPU 生命周期：资源释放必须等 GPU 完成，尺寸变化先重建再清理。
* UI 布局：配置面板固定 label 宽度，必要时拆行，避免竖排。
* UI 组件：新增参数后必须全量更新调用点，避免漏传导致编译失败。
* 管线元信息：单一入口/单一派生，附件与回退策略统一解析。
* 详细经验见 `.agents/skills/refactor-playbook/references/refactor-notes.md`。
...
* 严格遵循各语言社区的惯用写法与最佳实践（C++、Rust、Go、Python 等）。
* 主动使用 linter 等自动化工具保证代码风格的一致性和正确性。
* 主动留意并指出以下“坏味道”：
  * 重复逻辑 / 复制粘贴代码；
  * 模块间耦合过紧或循环依赖；
  * 改动一处导致大量无关部分破坏的脆弱设计；
...
* 命名与格式：
  ...
  * Python：遵循 PEP 8；
  * 其他语言遵循对应社区主流风格。
* 在给出较大代码片段时，默认该代码已经过对应语言的自动格式化工具处理（如 `cargo fmt`、`gofmt`、`ruff`、`prettier`、`eslint` 等）。
* 注释：
  * 仅在行为或意图不明显时添加注释；
...
2. 是否在浪费篇幅解释 JoeyTeng 已经知道的基础知识？
...
* 请尽量使用书面化的语言，减少口语化的语言。不要长期在回答种使用同一种写作风格。
* Commit 消息格式：要求多行消息时，用 git commit -F 或多次 -m，避免把 \n 直接写进消息。
* GPG 签名：所有提交使用 -S；若需要改消息，请明确允许 --amend。
* 日志精简：当只需要关键证据时，优先贴最相关的 5–10 行并标注“关键行”，避免整份日志。
* 依赖/工具版本：涉及构建工具（Xcode/meson/python）时，优先写清路径与版本，减少往返。
